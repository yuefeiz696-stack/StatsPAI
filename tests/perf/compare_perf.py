...
def render_tex() -> str:
    ...
    for est, cfg in ESTIMATORS.items():
        ...
        ratio_str = (
            f"$\\mathbf{{{ratio_max:.1f}\\times}}$ {{\\footnotesize sp faster}}"
            if ratio_max and ratio_max > 1.5 else
            f"$\\mathbf{{{1/ratio_max:.1f}\\times}}$ {{\\footnotesize R faster}}"
    ...
    return (
        ...
        "Each row reports median wall-clock time at the largest sample size in the sweep, "
        "across 3 or 5 reps with one warmup, on identical seed-fixed DGPs. "
        "The ratio column is $R/\\text{sp}$ at the largest $N$; bold marks decisive $\\ge 1.5\\times$ wins.}\n"
